 ...
 * Vertical sticky list on xl+ (the command-center 3-zone breakpoint),
 * horizontal scroll strip below.
...
export function CategoryRail({
  ...
  return (
    <nav aria-label="Resource categories" className={className}>
      <div className="mb-2 hidden items-center gap-2 px-1 xl:flex">
      ...
      <div
        className={cn(
          ...
          "xl:max-h-[calc(100vh-180px)] xl:flex-col xl:overflow-y-auto xl:overflow-x-visible xl:pb-0 xl:pr-1"
        )}
      >
        {item(null, "All resources", totalCount)}
        {isLoading && categories.length === 0
          ? Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="h-9 w-28 shrink-0 animate-pulse rounded-lg bg-surface-2 xl:w-full" />
            ))
