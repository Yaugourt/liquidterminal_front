    ...
    <Card className={`bg-[#051728E5] border-2 border-[#83E9FF4D] shadow-[0_4px_24px_0_rgba(0,0,0,0.25)] ${className}`}>
      <div className="p-3">
        <Tabs defaultValue="orderbook" className="w-full">
          <TabsList className="grid grid-cols-2 mb-2 bg-[#112941] border border-[#83E9FF33] h-8">
            <TabsTrigger value="orderbook" className="text-white text-xs data-[state=active]:bg-[#83E9FF4D] py-1">Order Book</TabsTrigger>
            <TabsTrigger value="trades" className="text-white text-xs data-[state=active]:bg-[#83E9FF4D] py-1">Trades</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="orderbook" className="mt-2">
            <div className="space-y-2">
              ...
              <div className="h-[402px] overflow-y-auto pr-1 scrollbar-thin">
                ...
                {/* Asks (Sell orders) - 6 ordres */}
                <div className="space-y-1 mb-3">
                  {displayAsks.slice(0, 6).reverse().map((ask, index) => (
                    <div key={`ask-${index}`} className="grid grid-cols-3 gap-2 text-xs hover:bg-[#83E9FF10] py-1 rounded px-1">
                      <span className="text-[#F87171]">${formatPrice(ask.px)}</span>
                      <span className="text-white text-right">{formatSize(ask.sz)}</span>
                      <span className="text-gray-400 text-right">{formatSize(asksCumulativeMap.get(ask.px) || 0)}</span>
                    </div>
                  ))}
                </div>

                {/* Spread */}
                <div className="border-y border-[#83E9FF33] py-1 text-center mb-2 mx-1">
                  <span className="text-xs text-gray-400">Spread: </span>
                  <span className="text-xs text-white">
                    {spread.absolute > 0 ? (
                      `${spread.absolute.toFixed(1)} (${spread.percentage.toFixed(4)}%)`
                    ) : (
                      'N/A'
                    )}
                  </span>
                ...
                {/* Bids (Buy orders) - 6 ordres */}
                <div className="space-y-1">
                  {displayBids.slice(0, 6).map((bid, index) => (
                    <div key={`bid-${index}`} className="grid grid-cols-3 gap-3 text-xs hover:bg-[#83E9FF10] py-1 rounded px-1">
                      <span className="text-[#4ADE80]">${formatPrice(bid.px)}</span>
                      <span className="text-white text-right">{formatSize(bid.sz)}</span>
                      <span className="text-gray-400 text-right">{formatSize(bidsCumulative[index])}</span>
                    </div>
                  ))}
                </div>
              </div>
