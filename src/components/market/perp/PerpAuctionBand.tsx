...
/**
 * "Perp deploy auctions" band for /market/perp — live HIP-3 perp-DEX
 * dutch-auction state, the gas actually paid over the last 90d, and the latest
 * deploys. Links to /market/perp/auction for the full history.
 *
 * Mirrors SpotAuctionBand, but the past-deploy feed is `usePastAuctionsPerp`
 * (Hypurrscan `pastAuctionsPerp`, `Date` timestamps + `maxGas` in HYPE).
 */
export function PerpAuctionBand() {
  const { auctionState } = usePerpAuctionTiming();
  const { auctions, isLoading: auctionsLoading } = usePastAuctionsPerp();
  const [now] = useState(() => Date.now());

  const sorted = useMemo(
    () => [...auctions].sort((a, b) => b.time.getTime() - a.time.getTime()),
    [auctions]
  );
  ...
  const gasEvents = useMemo(
    () =>
      sorted
        .filter(
          (a) =>
            a.time.getTime() >= now - 90 * DAY_MS &&
            a.maxGas !== null &&
            Number.isFinite(a.maxGas) &&
            (a.maxGas as number) > 0
        )
        .map((a) => ({ time: a.time.getTime(), value: a.maxGas as number })),
    [sorted, now]
  );
  ...
  const stats7d = useMemo(() => {
    const recent = sorted.filter(
      (a) =>
        a.time.getTime() >= now - 7 * DAY_MS &&
        a.maxGas !== null &&
        Number.isFinite(a.maxGas) &&
        (a.maxGas as number) > 0
    );
    const avg = recent.length
      ? recent.reduce((s, a) => s + (a.maxGas as number), 0) / recent.length
      : 0;
  ...
  const peak = useMemo(() => {
    ...
    const auction = sorted.find((a) => a.time.getTime() === max.time);
    return { ...max, name: auction?.symbol ?? "?" };
  }, [gasEvents, sorted]);
            ...
            <ModuleTableRow
              ...
              cells={[
                <ModuleAsset
                  ...
                  kind="auto"
                  name={a.symbol}
                  sub={a.dex ? `${a.dex} · ${shortAddr(a.user)}` : shortAddr(a.user)}
                />,
                <span
                  key="g"
                  className={`mono ${
                    a.maxGas !== null && a.maxGas > auctionState.floorPrice
                      ? "font-medium text-gold"
                      : "text-text-secondary"
                  }`}
                >
                  {a.maxGas !== null ? compactHype(a.maxGas) : "—"}
                </span>,
                <span key="w" className="mono text-[11px] text-text-tertiary">
                  {fmtAgo(a.time.getTime(), now)}
                </span>,
              ]}
            />
          ))}
