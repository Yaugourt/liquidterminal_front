...
import { OrderTabButtons, OrderTableContent } from "./orders";
...
export function OrdersTable({ address }: OrdersTableProps) {
  ...
  // Hook pour les open orders (logique métier intégrée)
  const { 
    data: openOrders, 
  ...
  // Hook pour les TWAP orders (logique métier intégrée)
  const { 
    orders: twapOrders, 
