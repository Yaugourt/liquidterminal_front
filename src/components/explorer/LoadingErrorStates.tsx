"use client";

import { Header } from "@/components/Header";

interface LoadingStateProps {
  message?: string;
}

export function LoadingState({ message = "Loading..." }: LoadingStateProps) {
  return (
    <div className="min-h-screen">
      <Header customTitle="Explorer" />
      <div className="min-h-screen p-4 lg:p-12 space-y-6">
        <div className="text-white text-center flex flex-col items-center justify-center gap-4">
          <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-[#83E9FF]"></div>
          <p>{message}</p>
        </div>
      </div>
    </div>
  );
}

interface ErrorStateProps {
  message?: string;
}

export function ErrorState({ message = "An error occurred" }: ErrorStateProps) {
  return (
    <div className="min-h-screen">
      <Header customTitle="Explorer" />
      <div className="min-h-screen p-4 lg:p-12 space-y-6">
        <div className="text-red-500 text-center flex flex-col items-center justify-center gap-4">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="48"
            height="48"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
          >
            <circle cx="12" cy="12" r="10" />
            <line x1="12" y1="8" x2="12" y2="12" />
            <line x1="12" y1="16" x2="12.01" y2="16" />
          </svg>
          <p>{message}</p>
        </div>
      </div>
    </div>
  );
} 