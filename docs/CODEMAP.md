...
- `/explorer/address/[address]` — `AddressAnalyticsLayout` (also reused by `/market/tracker/wallet/[address]`) · explorer variant = `AddressDigest` (`components/address/digest`): one `KpiRibbon` + `WalletProfileCard` (balances / markets / edge / risk + a single by-market table) fed by `useAddressDigest`, which opens every feed once — HL `portfolio` (`pnlHistory`, deposits excluded), `clearinghouseState` (liq distance), Hypedexer `/indexer/users/:user/*` (lifetime via `start_time`, the default is 7d), `/indexer/funding/userFunding/summary`, local DB `/liquidations?user=`; `classifyWallet` = heuristic archetype pill (tooltip shows the rule that fired) · tracker variant keeps PortfolioStats + PerformanceChart + the standalone Scorecard / Concentration / Funding / HyperEVM cards · per-tab sources (RPC, indexer, backend staking).
