...
| Auth | `src/contexts/auth.context.tsx` + `src/services/auth/` | Privy OAuth → POST `/auth/login` → JWT in interceptor; roles USER < MODERATOR < ADMIN (`src/lib/roleHelpers.ts`, `<ProtectedAction>`) · the Privy SDK (~380 KB gz) loads on demand (`services/auth/privy`: facade `usePrivy`/`useLogin`/`useModalStatus`, never `@privy-io/react-auth` directly — ESLint) · requests sent before it has loaded carry no JWT unless flagged `awaitAuth` (optional-auth endpoints, auth endpoints fetched on mount) |
...
| Shell | `src/app/(app)/layout.tsx` (server) + `src/components/Providers.tsx` + `src/components/AppShell.tsx` | Providers: Auth → Xp → XpNotification → Toaster, with `<LazyPrivy>` mounted next to the tree (loads at idle, or at once when a Privy session is stored) · AppShell (client): Sidebar, Header, mobile menu; `LegalFooter` rendered by the server layout · Cmd+K palette mounted once in the root layout (`components/search/GlobalSearch.tsx`, lazy palette) |
...
| ESLint guardrails | `eslint.config.mjs` | hex ban (use tokens/chartPalette), raw `<Table>` ban (use TypedDataTable), grid-cols-px table ban, common-barrel-only imports, `<Checkbox>` over raw input, no runtime `@privy-io/*` import outside `services/auth/privy/PrivyRoot.tsx` |
